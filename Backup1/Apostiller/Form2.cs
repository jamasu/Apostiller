    ...
    public partial class Form2 : Form
    {
        ...
        private void dataGridView1_CellContentClick(Object sender, DataGridViewCellEventArgs e)
        {
            ...
            try
            {
                ...
                {
                    using (PreferenceDbDataContext prf = new PreferenceDbDataContext())
                    {
                        try
                        {
                            var pref = prf.preferences.Single();
                            prefCity = pref.pref_city;
                            prefCounty = pref.pref_county;
                        }
                        catch (Exception exe)
                        {
                            Console.WriteLine(exe.Message);
                    ...
                    using (PersonDataContext pdc = new PersonDataContext())
                    {
                        try
                        {
                            ...
                            ap.Print(ap.printDocument1.PrinterSettings.PrinterName, apos.name, Convert.ToDateTime(apos.date).ToString("dd.MM.yyyy"), apos.title, apos.capacity, apos.doc, apos.about, apos.sig, apos.country, apos.office, apos.ID.ToString(), prefCity, prefCounty,apos.fullname);
